  ...
  const {id} = useParams();
    ...
    useEffect(() => {
      ...
      try{
        ...
        setDescription(data.courseDetail.description);
        setPrice(data.courseDetail.price);
        setImage(data.courseDetail.image.url);
        setImagePreview(data.courseDetail.image.url);
       setLoading(false);
      }catch(error){
        console.log(error);
        toast.error("Error to Fetch Course Data");
        setLoading(false);
      }
    }
    fetchCourseData();
